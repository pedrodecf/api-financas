...
export class DeleteUseCase {
  ...
  async execute({
    ...
    return this.categoriasRepository.$transaction(async (tx) => {
      ...
      const categoriaToBeDeleted = await this.categoriasRepository.findById(
        id,
        usuarioId,
        tx
      );
      ...
      const haveAnyTransaction =
        await this.transacoesRepository.findByCategoriaId(id, usuarioId, tx);
      if (haveAnyTransaction.length > 0)
        throw new NaoAcessivelError(
